...
## ・genre_goods.csv                                         
##    -本店の管理をしているECBeingからダウンロードしたアイテムのジャンルリストファイル                        
...
my $genre_goods_file_name="genre_goods.csv";
...
while ($input_dir_file_name = readdir(INPUT_DIR)){
	...
	if($input_dir_file_name eq $genre_goods_file_name) {
...
if (!$genre_goods_file_find) {
	#genre_goods.csvファイルが存在しない
	output_log("ERROR!! Not exist $genre_goods_file_name.\n");
}
if (!$dl_item_file_find) {
...
if (!$goods_file_find || !$genre_goods_file_find || !$dl_item_file_find || $dl_item_file_multi || !$dl_select_file_find || $dl_select_file_multi) {
	#入力ファイルが不正
	exit 1;
}
...
$genre_goods_file_name = "$input_dir"."/"."$genre_goods_file_name";
if (!open(GENRE_GOODS_FILE, "< $genre_goods_file_name")) {
	output_log("ERROR!!($!) $genre_goods_file_name open failed.\n");
	exit 1;
}
...
# CSV項目の出力
my @csv_item_name=("登録体系","商品コード","カテゴリ名","商品名","販売価格","サイズ","カラー","在庫数","カテゴリ");
...
while($goods_line = <GOODS_FILE>){
	...
	my $glober_goods_code_5=substr($glober_goods_code, 0, 5);
	# itemファイルの読み出し
	seek(ITEM_FILE,0,0);
	# 1行目は読み飛ばす
	...
	if($find_flag != 1) {
		...
		while($select_line = <SELECT_FILE>){	
			...
			my $rakuten_code=$rakuten_select_code[1];
			if(length($rakuten_code = 5)){
				my $rakuten_code_addition=$rakuten_select_code[8];
				my $code="$rakuten_code$rakuten_code_addition";
				my $rakuten_code_7=substr($code, 0, 7);
				my $glober_code_7=substr($glober_goods_code, 0, 7);
				my $rakuten_code_5=substr($code, 0, 5);
				my $glober_code_5=substr($glober_goods_code, 0, 5);
				if ($code==$glober_goods_code) {
				#5桁+4桁で合致した場合はflagを立てる
				$find_flag=1;
				last;
				}else if($rakuten_code_5 == $glober_code_5){
						$entry_kinds = "A"
						last;
				}end if{
				$entry_kinds = "N"
			...
			} else if(length($rakuten_code = 7)){
				my $rakuten_code_addition=$rakuten_select_code[6];
				my $code="$rakuten_code$rakuten_code_addition";
				if ($code==$glober_goods_code) {
	...
	if($find_flag != 1){
			$output_sabun_csv->combine($entry_kinds) or die $output_sabun_csv->error_diag();
			...
			print SABUN_FILE $output_sabun_csv->string(), ",";
			# カテゴリ名,商品名,在庫数の抽出
			$output_sabun_csv->combine($glober_genre) or die $output_sabun_csv->error_diag();
			print SABUN_FILE $output_sabun_csv->string(), "\n";
		}
}
