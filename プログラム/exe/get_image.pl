...
while($sabun_line = $input_sabun_csv->getline($input_sabun_file_disc)) {
	...
	# 商品コードの上位7桁を切り出し
	my $entry_goods_code_index=index($entry_goods_code_tmp, "\"",7);
	my $entry_goods_code=substr($entry_goods_code_tmp, 0, $entry_goods_code_index-1);
	my $comp_count=0;
...
#	my $goods_line=<GOODS_FILE>;
	...
	while($goods_line = $input_goods_csv->getline($input_goods_file_disc)){
		...
		# 商品コードの上位7桁を切り出し
		my $comp_code_index=index($comp_code_tmp, "\"",7);
		my $comp_code=substr($comp_code_tmp, 0, $comp_code_index-1);
		if ($entry_goods_code eq $comp_code) {
			$comp_count++;
		}
	}
	# SKUの場合は7桁を登録
...
foreach my $get_image_code ( @goods_code_list ) {
	...
	if (length($get_image_code) == 7) {
		# 画像取得の為に11を付与して9桁にする
		$correct_image_code="$get_image_code"."11";
	}
	else {
	...
	while(!$rtn) {
		...
		if ($cnt==1) {
			...
			if ($cnt>= 6) {
				copy( "$r_image_dir/$get_image_code\_$cnt.jpg", "$y_s_over6_image_dir/$get_image_code\_$cnt.jpg" ) or die("ERROR!! $y_s_over6_image_dir/$get_image_code\_$cnt.jpg copy failed.");					
				&image_resize("$y_s_over6_image_dir/$get_image_code\_$cnt.jpg", "$y_s_over6_image_dir/$get_image_code\_$cnt.jpg", 600, 600, 0);
				# ZIPファイル化
				&add_y_s_over6_zip("$y_s_over6_image_dir/$get_image_code\_$cnt.jpg", "$get_image_code.jpg");
			}
			else {
				# Yahoo用画像フォルダにコピー
				copy( "$r_image_dir/$get_image_code\_$cnt.jpg", "$y_image_dir/$get_image_code\_$cnt.jpg" ) or die("ERROR!! $y_image_dir/$get_image_code\_$cnt.jpg copy failed.");
				&image_resize("$y_image_dir/$get_image_code\_$cnt.jpg", "$y_image_dir/$get_image_code\_$cnt.jpg", 600, 600, 0);
				&add_y_zip("$y_image_dir/$get_image_code\_$cnt.jpg", "$get_image_code\_$cnt.jpg");
			}
			copy( "$r_image_dir/$get_image_code\_$cnt"."s.jpg", "$y_s_over6_image_dir/$get_image_code\_$cnt"."s.jpg" ) or die("ERROR!! $y_s_over6_image_dir/$get_image_code\_$cnt"."s.jpg copy failed.");
			&add_y_s_over6_zip("$y_s_over6_image_dir/$get_image_code\_$cnt"."s.jpg", "$get_image_code\_$cnt"."s.jpg");
		}
	}
	# 画像数をファイルに書き込み
	$cnt--;
